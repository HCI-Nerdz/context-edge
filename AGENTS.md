...
- **Variant C — Path Edge:** breadcrumb as color series on one edge (Top labels or Side marks); full edge length; Rest/Live rows with Color/Subtle style + expandable zone (`/demos/path-edge/` shared workshop mount; Solid/React/Svelte host the same suite)
