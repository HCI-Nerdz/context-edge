...
- **Variant B — Modal Edge:** edge = current node; scoped View Transitions slide the sheet to reveal colored ancestry layers (`src/lib/modal-vt.ts`; `/demos/modal-edge/` Rest/Live on every implementation)
- **Variant C — Path Edge:** breadcrumb as color series on one edge (Top labels or Side marks); full edge length; Still/Live rows with Color/Subtle style + stage edge drag-resize (`/demos/path-edge/` shared workshop mount; Solid/React/Svelte host the same suite)
